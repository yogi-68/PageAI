-- Restrict users from updating billing-sensitive profile columns via RLS
-- Migration: 20260622_protect_billing_profile_fields
...
BEGIN
  IF auth.uid() = OLD.id AND auth.role() = 'authenticated' THEN
    IF NEW.plan IS DISTINCT FROM OLD.plan
       ...
       OR NEW.monthly_message_count IS DISTINCT FROM OLD.monthly_message_count THEN
